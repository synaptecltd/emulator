...
func TestTrendAnomalyGetSign(t *testing.T) {
	params := TrendParams{Name: "test", Duration: 1.0}

	t.Run("NotInverted", func(t *testing.T) {
		params.InvertTrend = false
		trend, err := NewTrendAnomaly(params)
		require.NoError(t, err)
		assert.Equal(t, 1.0, trend.getSign())
	})

	t.Run("Inverted", func(t *testing.T) {
		params.InvertTrend = true
		trend, err := NewTrendAnomaly(params)
		require.NoError(t, err)
		assert.Equal(t, -1.0, trend.getSign())
	})
